...
class UMDAc (Evolutionary):
    ...
    def select(self, values, x):
        return self.selection_function(self.coefficients[x], values, self.descendant_size, 2)

    def model_estimation(self, coefficients, x):
        self.mean[x] = np.mean(coefficients, axis=0)
        self.deviation[x] = np.std(coefficients, axis=0) * 0.9
    ...
    def run(self, x):
        ...
        for i in range(self.iters):
            print(f'n: {x}, iter: {i}')
            selected_coefficients = self.select(values, x)
            self.model_estimation(selected_coefficients, x)
            self.coefficients = self.random_coefficients(x)
